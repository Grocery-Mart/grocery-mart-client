import {
  MenuProduct101,
  MenuProduct102,
  MenuProduct11,
  MenuProduct111,
  MenuProduct112,
  MenuProduct12,
  MenuProduct121,
  MenuProduct122,
  MenuProduct131,
  MenuProduct132,
  MenuProduct141,
  MenuProduct142,
  MenuProduct151,
  MenuProduct152,
  MenuProduct162,
  MenuProduct21,
  MenuProduct22,
  MenuProduct31,
  MenuProduct32,
  MenuProduct41,
  MenuProduct42,
  MenuProduct51,
  MenuProduct52,
  MenuProduct61,
  MenuProduct62,
  MenuProduct71,
  MenuProduct72,
  MenuProduct81,
  MenuProduct82,
  MenuProduct91,
  MenuProduct92,
} from '~/components/Icons';

export const DataDepartments = [
  {
    icon1: <MenuProduct21 />,
    icon2: <MenuProduct22 />,
    title: 'Điện tử',
    data: [
      {
        title: 'PC, Laptop',
      },
      {
        title: 'Mobile',
      },
      {
        title: 'Tivi thông minh',
      },
      {
        title: 'Đồng hồ thông minh',
      },
      {
        title: 'Màn hình máy tính',
      },
      {
        title: 'Tản nhiệt, Fan, LED',
      },
      {
        title: 'Loa, tai nghe',
      },
      {
        title: 'Phụ kiện PC, Laptop, Mobile',
      },
      {
        title: 'Webcam, Camera',
      },
      {
        title: 'Thiết bị văn phòng, hội nghị',
      },
    ],
  },

  {
    icon1: <MenuProduct41 />,
    icon2: <MenuProduct42 />,
    title: 'Nhà & Nội thất, Thiết bị',
    data: [
      {
        title: 'Phòng khách',
      },
      {
        title: 'Phòng ngủ',
      },
      {
        title: 'Trang trí nội thất',
      },
      {
        title: 'Dụng cụ nhà bếp',
      },
      {
        title: 'Trang trí văn phòng',
      },
    ],
  },
  {
    icon1: <MenuProduct11 />,
    icon2: <MenuProduct12 />,
    title: 'Gia đình & Cuộc sống',
    data: [
      {
        title: 'Nội thất',
      },
      {
        title: 'Đồ dùng nhà bếp',
      },
      {
        title: 'Văn phòng phẩm',
      },
      {
        title: 'Phòng tắm',
      },
      {
        title: 'Kho',
      },
      {
        title: 'Dụng cụ vệ sinh',
      },
    ],
  },
];

export const DataGrocery = [
  {
    icon1: <MenuProduct61 />,
    icon2: <MenuProduct62 />,
    title: 'Bánh mì',
    data: [
      {
        title: 'Sản phẩm mới',
      },
      {
        title: 'Bánh mì cắt lát',
      },
      {
        title: 'Bánh cuốn & bánh bao',
      },
      {
        title: 'Bánh nướng',
      },
      {
        title: 'Bánh quy & bánh hạch nhân',
      },
    ],
  },
  {
    icon1: <MenuProduct71 />,
    icon2: <MenuProduct72 />,
    title: 'Đồ ăn nhẹ',
    data: [
      {
        title: 'Hamburger',
      },
      {
        title: 'Khoai tây chiên',
      },
      {
        title: 'Bắp rang',
      },
      {
        title: 'Salsa & nước chấm',
      },
      {
        title: 'Bánh quy',
      },
    ],
  },
  {
    icon1: <MenuProduct91 />,
    icon2: <MenuProduct92 />,
    title: 'Kẹo',
    data: [
      {
        title: 'Sô cô la',
      },
      {
        title: 'Kẹo cao su',
      },
      {
        title: 'Kẹo cứng',
      },
      {
        title: 'Kẹo mềm',
      },
      {
        title: 'Kẹo cao su',
      },
    ],
  },
  {
    icon1: <MenuProduct101 />,
    icon2: <MenuProduct102 />,
    title: 'Đồ uống',
    data: [
      {
        title: 'Nước ngọt',
      },
      {
        title: 'Nước trái cây',
      },
      {
        title: 'Đồ uống thể thao',
      },
      {
        title: 'Nước tăng lực',
      },
      {
        title: 'Trà',
      },
      {
        title: 'Socola nóng & cacao',
      },
    ],
  },
  {
    icon1: <MenuProduct51 />,
    icon2: <MenuProduct52 />,
    title: 'Sản phẩm tươi sống',
    data: [
      {
        title: 'Hoa quả tươi',
      },
      {
        title: 'Rau sạch',
      },
      {
        title: 'Sản phẩm hữu cơ',
      },
    ],
  },
  {
    icon1: <MenuProduct81 />,
    icon2: <MenuProduct82 />,
    title: 'Thực phẩm đông lạnh',
    data: [
      {
        title: 'Thịt đông lạnh',
      },
      {
        title: 'Bữa ăn & đồ ăn nhẹ đông lạnh',
      },
    ],
  },
  {
    icon1: <MenuProduct111 />,
    icon2: <MenuProduct112 />,
    title: 'Cà phê',
    data: [
      {
        title: 'Cà phê hòa tan',
      },
      {
        title: 'Cà phê nguyên chất',
      },
      {
        title: 'Cà phê xay sẵn',
      },
      {
        title: 'Dụng cụ pha cafe',
      },
    ],
  },
];

export const DataBeauty = [
  {
    icon1: <MenuProduct121 />,
    icon2: <MenuProduct122 />,
    title: 'Quần áo nam',
    data: [
      {
        title: 'Áo Sơ Mi',
      },
      {
        title: 'Quần Jean & Quần vải',
      },
      {
        title: 'Áo khoác ngoài',
      },
      {
        title: 'Áo thun',
      },
      {
        title: 'Quần áo thường ngày',
      },
    ],
  },
  {
    icon1: <MenuProduct131 />,
    icon2: <MenuProduct132 />,
    title: 'Quần áo nữ',
    data: [
      {
        title: 'Váy đầm',
      },
      {
        title: 'Váy ngắn',
      },
      {
        title: 'Áo sơ mi',
      },
      {
        title: 'Túi xách',
      },
      {
        title: '',
      },
    ],
  },
  {
    icon1: <MenuProduct141 />,
    icon2: <MenuProduct142 />,
    title: 'Giày dép',
    data: [
      {
        title: 'Giày nam',
      },
      {
        title: 'Giày nữ',
      },
      {
        title: 'Giày thể thao',
      },
      {
        title: 'Giày da & Bốt',
      },
      {
        title: 'Dép',
      },
    ],
  },
  {
    icon1: <MenuProduct151 />,
    icon2: <MenuProduct152 />,
    title: 'Phụ kiện thời trang',
    data: [
      {
        title: 'Thắt lưng',
      },
      {
        title: 'Mũ & Mũ lưỡi trai',
      },
      {
        title: 'Khăn quàng cổ',
      },
      {
        title: 'Găng tay',
      },
      {
        title: 'Kính râm',
      },
      {
        title: '',
      },
    ],
  },
  {
    icon1: <MenuProduct51 />,
    icon2: <MenuProduct162 />,
    title: 'Quần áo theo mùa',
    data: [
      {
        title: 'Trang phục mùa hè',
      },
      {
        title: 'Trang phục mùa thu',
      },
      {
        title: 'Trang phục đông',
      },
      {
        title: 'Quần áo mưa',
      },
    ],
  },
  {
    icon1: <MenuProduct31 />,
    icon2: <MenuProduct32 />,
    title: 'Ưu đãi thời trang',
    data: [
      {
        title: 'Quần áo',
      },
      {
        title: 'Giày dép',
      },
      {
        title: 'Phụ kiện',
      },
      {
        title: 'Túi',
      },
      {
        title: 'Trang sức',
      },
    ],
  },
];
